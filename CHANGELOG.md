...
- **Detached-stream cancellation watcher: in-process notification + relaxed poll fallback.** The watcher now subscribes to a per-engine in-process run-status notifier (`internal/application/runtime/runtime_status_notify.go`): every engine settle helper (`markRunFailedMode` / `markRunCancelled` / `ensureRunPaused` / `persistRunCompleted`) broadcasts a wake hint after its persistence attempt, so a same-process cancellation/failure/completion of a detached run aborts the blocking LLM call immediately instead of within one poll tick. The hint never decides — the watcher still re-loads the snapshot and confirms the status, and cancellations persisted by OTHER processes still rely on the poll ticker, which remains the correctness mechanism. With the fast path in place, the default poll interval is relaxed from 250ms to 2s (~8x less steady-state repository read pressure per detached run); the `detached_cancellation_poll_interval` scenario key is unchanged.
- **BREAKING — autonomous iteration checkpoints are incremental.** `StepOutputs["auto:iter:<n>"]` now stores an envelope (`{"format":"full"|"delta","base":<n>,"messages":[...]}`) holding only the messages appended since the previous boundary instead of the whole conversation, so iteration n costs O(new messages) instead of O(transcript) — the per-iteration write no longer grows linearly with run length. `ResumeAutonomousFromIteration` (and `Framework.RetryFailedRun`) rebuilds the conversation by folding the envelopes in order (`delta` appends, `full` replaces — the fallback when context compaction shrank the transcript below the recorded baseline); crash recovery stays at-least-once at iteration granularity. Snapshots written by older versions (a bare `[]llm.Message`) fail decode with an explicit error; there is no in-repo migration tooling — the postgres migrations version the snapshot TABLE only, and step-output payloads are opaque JSON to every store.
- **BREAKING — tenant-strict mode is now the default.** `AuthorizeTenant` / `LoadAuthorized` (and the `pkg/async` / blob-scope equivalents) fail closed (`ErrTenantRequired`) when a principal-less caller touches tenant-partitioned data (stamped `RunSnapshot` / `Job`, tenant-scoped blob refs). Unpartitioned legacy data (unstamped snapshots/jobs, legacy blob IDs) stays open, so single-tenant and internal paths are unaffected. The old opt-in `runstate.ContextWithTenantStrictMode` is removed; trusted maintenance callers opt OUT with the new `runstate.ContextWithTenantPermissive(ctx)`. The HTTP layers no longer wrap handlers in strict mode (redundant). `ScopeListFilter` / `ScopeJobFilter` reject principal-less explicit tenant scopes but keep the empty global maintenance view.
- **BREAKING — stream caller-gone fallback is on by default.** `Framework.Stream` now applies `DefaultStreamCallerGoneTimeout` (10 minutes) to non-detached streams whose caller stops consuming without cancelling; `WithStreamCallerGoneTimeout(d <= 0)` explicitly disables it (previously `0` was the default and meant "off").

### Added

- **Approval cache + deny breaker ride the pause checkpoint**: the tool-approval pause now persists the run-scoped approval decisions and the HITL deny-breaker count as `checkpoint_approvals` / `checkpoint_deny_count` next to `checkpoint_tool_counts`, and `ContinueAfterCheckpoint` restores them — a run that migrates nodes (pause/resume to another worker, reaper-driven crash recovery) keeps its "remembered" approvals (no repeated HITL prompts) and its consecutive-deny budget. `pkg/toolorch` gains the optional `RunStateExporter` interface (`ExportRun` / `ImportRun`), implemented by `MemoryApprovalStore`; `DenyBreaker` gains matching `ExportRun` / `ImportRun`. Custom `ApprovalStore` implementations without the interface degrade to the previous process-local behavior.
- **Token budget circuit breaker**: scenario runtime gains `max_total_tokens` (YAML key synced; zero disables). After every LLM turn of the tool loop the run's accumulated provider-reported total (checkpointed across pause/resume via `checkpoint_usage`, so a resumed run cannot spend the budget twice) is compared against the budget; exceeding it fails the run with the `runtime.ErrTokenBudgetExceeded` sentinel and `termination_reason=budget_exceeded` (`core.TerminationReasonBudgetExceeded`). Providers that never report usage never trip it.

- **Async event emission with backpressure**: engine and facade event delivery now share one emission pipeline (`internal/application/emit`). Non-lifecycle events are queued (single dispatcher goroutine, in-order) so a slow sink no longer stalls the tool loop; a full queue drops events and counts them via the new `agentflow_runtime_events_dropped_total` metric plus rate-limited warnings (same drop semantics as the stream tee). Critical lifecycle events (`RunStarted`/`RunCompleted`/`RunFailed`/`RunCancelled`/`RunPaused`/`RunResumed` — `RunStarted`/`RunResumed` are now retried too) bypass the queue: synchronous delivery with bounded retries. Capacity is configurable via `WithEventQueueCapacity` (default 1024); `Framework.Close` drains the queue with a bounded 2s wait, and `Engine.Close` does the same for standalone engines.

- **`Framework.ContinueRunWithToken(ctx, runID, token)`**: credential-checked variant of `ContinueRun` — the HITL pause token is verified (HMAC signature, run-ID binding, TTL via the configured `TokenSigner`, or the gate's `core.PauseTokenDecoder`) before the approved-but-not-continued run is driven forward, so knowing a run ID alone is no longer enough on surfaces that adopt it. `ContinueRun` is unchanged and remains the trusted operator recovery path.
- **Attributable termination reasons**: terminal run payloads (`RunTerminalPayload`) carry `termination_reason` (`completed` / `max_steps_exceeded` / `timeout` / `cancelled` / `lease_lost` / `llm_error` / `error`, see `core.TerminationReason*`). The autonomous loop's step-budget exhaustion now wraps the `runtime.ErrMaxStepsExceeded` sentinel (message text unchanged), and `RunCancelled` events no longer emit a nil payload.
- **Usage-driven context compaction**: the runtime keeps a per-run usage tracker (checkpointed as `checkpoint_usage`, old snapshots decode to zero), and context preparation prefers the previous call's provider-reported token count over the heuristic estimate for trigger decisions (`contextwindow.Manager.PrepareWithOptions` with `PrepareOptions.KnownInputTokens`).
- **Context-overflow recovery**: `llm.APIError` gains a `Code` field and `IsContextLengthExceeded()` (OpenAI and Anthropic gateways fill the code on both unary and streaming error paths). A `context_length_exceeded` failure triggers one recovery compaction + retry per run; a second overflow fails the run.
- **Conversation-format invariants**: empty LLM turns (no content, no tool calls, not a length cutoff) are re-sampled up to 3 times before failing with a diagnostic error. Tool arguments repaired from malformed JSON now mark the assistant message (`tool_args_normalized`), emit a `ToolArgsNormalized` diagnostic event, and append the original parse error to subsequent input-validation feedback.
- **Pluggable tool inspector pipeline** (`pkg/toolinspect`): the runtime's tool-dispatch decision gates (agent whitelist, scenario declaration, input-schema validation, approval cache, deny-without-gate, executor registry/resolution, security, doom-loop/rate-cap budget, governance) are now an ordered `toolinspect.Chain` with short-circuit semantics — order, `ToolDenied` event payloads, and audit records are unchanged. Hosts register custom inspectors with `WithToolInspectors(prepend, append...)`.
- **Deferral economics gate** (`pkg/toolcatalog`): `NewSnapshotWithDeferral` attaches a `DeferralPolicy{MinTools (default 8), MaxOverheadTokens}`; small catalogs are advertised in full, and approval-gated tools (`risky` / `always` / `pause`) are never deferred. Legacy snapshots keep unconditional deferral.
- **Feature contribution model** (`pkg/feature`): `WithFeatures(...)` wires host features contributing LLM middleware, tool inspectors, tool-loop hooks (`OnStepFinish` after each persisted step, panic-isolated), and stop conditions (their reason becomes the run's `termination_reason`). Collection is error-isolated per feature; `feature.UsageAccounting` is an optional built-in that accumulates per-step token usage to a host sink.
- **Dual-visibility message projection**: `WithDualVisibilityMessages(true)` makes context-window trimming mark trimmed messages `metadata["visibility"]="user"` instead of physically dropping them — events, memory, and checkpoints keep the full transcript (marks ride along via `checkpoint_messages`, so pause/resume needs no migration), while the OpenAI/Anthropic gateways send the model only the agent-visible projection. `pkg/llm` gains `MarkUserVisibleOnly` / `MarkAgentVisibleOnly` / `IsAgentVisible` / `IsUserVisible` / `AgentVisibleMessages` / `UserVisibleMessages`; `contextwindow.Manager` gains `WithMarkInsteadOfDrop` (default off, byte-identical behavior) and `Stats.MarkedMessages`; the `ContextPrepared` event payload reports `marked_messages`.
- **Stream Hub** (`stream_hub.go`): `StreamRun` sessions now live in a per-run hub (1024-frame replay ring, terminal-frame 30s grace period) and the new `Framework.AttachRunStream(ctx, runID, opts...)` subscribes late or additional consumers: ring replay first (with an `events_lost` gap marker on ring overflow), then live frames. A paused run is no longer a stream end — after `ResumeAndContinue`/`ContinueRun`, execution keeps publishing into the same session. Runs with no live session fall back to `WithEventStore` replay plus a synthetic done frame from persisted state. Subscriber channels are bounded (256) with the tee's backpressure contract: slow subscribers lose event frames (counted, surfaced via cumulative `events_lost` markers), token/terminal frames are never dropped. The primary `StreamRun` channel behavior is unchanged; `Framework.Close` detaches all subscribers.
- **`WithStreamCallerGoneTimeout(d)`**: fallback deadline for `Framework.Stream` callers that stop consuming the returned channel without cancelling ctx. Once a chunk stays undeliverable for `d` on a non-detached stream, the framework cancels the stream's execution context so the engine settles the run (cancelled) and the run lease is released, instead of blocking the stream forwarder, the engine goroutine and the lease renewer forever. The timer runs only while a chunk is blocked on delivery — a slow-but-reading consumer never triggers it, and a quiet engine is unaffected. Defaults to `DefaultStreamCallerGoneTimeout` (10 minutes; see the BREAKING note above); `d <= 0` disables. Detached streams (`StreamDetached` / `WithStreamDetached`) are exempt.

### Fixed

- **Second defect-hunt round (2026-08, F1–F7; see DEFECT_REPORT.md "第二轮猎查")**:
  - **F1 — `StreamRun` caller cancel no longer leaks the hub session.** The frame merger returned without a terminal frame when the caller's ctx was cancelled, so the run's hub session (replay ring included) lived forever, attached subscribers hung, and a detached run silently dropped every post-cancel frame with the tee. The merger now settles the session from a deferred guard: any exit that never delivered its closing frame publishes a terminal `error` frame (releasing subscribers and starting the grace-period reclamation). A detached run whose caller went away switches the merger to hub-only delivery — later tokens/events and the truthful terminal `done` frame keep flowing to `AttachRunStream` subscribers (the terminal snapshot load uses `context.WithoutCancel`, so a completed detached run is no longer misreported as an error).
  - **F2 — re-execution after a terminal frame gets a fresh hub session.** `ContinueRun` / `RetryFailedRun` (and the resume paths through `continueRun`) now go through `driveWithStreamSession`: a terminal session for the same run ID is replaced on entry (its subscribers detached, grace timer stopped) and the re-execution's events plus its Done/Error outcome are published to the new session — previously the re-run produced zero hub frames and an attach replayed the stale terminal stream. The event-store replay fallback no longer appends a synthetic `Done{Status:running}` frame for a run that is still Running; it ends after the stored events instead of misreporting a live run as finished.
  - **F3 — emit pipeline survives sink panics.** The dispatcher goroutine runs under `safecall.GoSafe`, each queued delivery recovers a host-sink panic per event (counted as dropped + warn log, mirroring the queue-full path), and the synchronous lifecycle path (`EmitWithLifecycleRetry`) converts a sink panic into an error for the caller — a panicking sink can no longer crash the process.
  - **F4 — iteration delta baselines are content-anchored.** The `auto:iter:<n>` delta envelope gains `base_hash` (sha256 of the prefix's last message at persist time). The writer degrades to a `full` snapshot when the recorded anchor does not match the current prefix (a compacted-then-regrown conversation passes the old length-only check but would fold back garbled), and the fold side rejects a delta whose anchor disagrees with the rebuilt prefix. Envelopes without the field fold with the historic length-only check.
  - **F5 — crash recovery folds across pause holes.** A tool-approval pause returns before the iteration-boundary persist, leaving its step number as a hole in the `auto:iter` chain; `loadAutonomousConversation` used to fail the run permanently on the missing key ("checkpoint chain has a gap") on every retry. The fold now tolerates missing keys and enforces integrity by content instead of contiguity: a `full` envelope re-anchors the chain, and a delta spanning the hole is validated by its base (and `base_hash`); a gap the next delta does not span still fails loudly.
  - **F6 — no silent event loss on the enqueue/Close race.** An enqueue whose channel send won the race against `Pipeline.Close`'s final sweep used to strand the event in the queue — never delivered, never counted. The enqueue rechecks `stopped` after a successful send and funnels stranded items (plus pending flush barriers) into the same sweep `Close` uses, so every event is accounted exactly once.
  - **F7 — approval-cache checkpoint import fails open.** A corrupt `checkpoint_approvals` payload (pause/resume path and iteration-envelope restore share `restoreApprovalCheckpointState`) no longer fails the run permanently: the import error degrades to a warn log plus an empty approval cache, matching the "store without `RunStateExporter`" degradation — the worst case is a repeated HITL prompt, never a stuck run.

- **Crash recovery no longer resets run-level loop state**: the autonomous iteration envelope (`StepOutputs["auto:iter:<n>"]`) gains a `state` snapshot carrying the same run-scoped state the tool-approval pause persists into checkpoint variables — the doom-loop/rate-cap tracker (`checkpoint_tool_counts`), the usage tracker (`checkpoint_usage`: accumulated token totals + context-recovery attempts), the approval cache (`checkpoint_approvals`), the deny-breaker count (`checkpoint_deny_count`), and the cumulative step/replan budgets. `ResumeAutonomousFromIteration` (and `Framework.RetryFailedRun`) restores them through the same decode/import helpers as `ContinueAfterCheckpoint`, so a reaper/recovery-driven resume keeps the `max_total_tokens` budget, doom-loop counts, remembered approvals and the replan limit instead of restarting them from zero — pause/resume and crash recovery now share one state-continuity semantic. Envelopes written before the field existed (one release back, when the envelope format itself was introduced) decode with no state and resume with zeroed trackers, the previous behavior; pre-envelope snapshots still fail decode with an explicit error.
- **Terminal-persist retry exhaustion now leaves a recoverable trail.** When `markRunFailedMode` / `markRunCancelled` exhaust all jittered CAS retries (persistent storage failure or an unmanaged concurrent writer), the engine no longer just logs a warning: it logs at error level, emits a new `RunTerminalPersistFailed` diagnostic event (`core.EventRunTerminalPersistFailed`, payload carries only the target status and save error), and best-effort stamps the `terminal_persist_failed` snapshot variable (`runstate.VarTerminalPersistFailed`, value = intended target status) so the abandoned-run reaper and operator inspection can distinguish "worker finished but could not settle" from a genuinely live Running run. The marker write is still an optimistic CAS — never a force-write over an actively advancing writer — and the stale-fence path (a superseded lease; the new owner settles the run) keeps its previous warn-only, single-attempt behavior. Releasing the run lease from the engine was not possible: the lease handle is owned by the facade's `holdRunLease` renewal loop, the engine only sees owner/token as context values.
- **`ensureRunPaused` retries stale-snapshot conflicts**: the paused-status save after `gate.Pause` now uses the same load→CAS→jittered-retry loop as `saveSnapshotWithRetry` (`ErrStaleFence` still passes through without retries), so a concurrent snapshot advance can no longer strand a run in `Running` after its pause token was issued. A gate that already persisted the Paused transition itself (the built-in gates do) makes the save a no-op: an already-Paused run is never rewritten, which would advance the version and supersede the just-issued pause token (`ErrTokenSuperseded` on resume).

- **Unconfirmed pause checkpoints can no longer be executed**: pause checkpoint writes now stamp `checkpoint_pending_pause` (`runstate.VarCheckpointPendingPause`), cleared only when the gate confirms an approval (the built-in CLI gate clears it atomically in the resume save; every Framework resume entry point covers custom gates). The engine refuses to continue a checkpoint that still carries the marker (fail closed — a crash between the checkpoint write and `gate.Pause` means nobody approved anything), and the abandoned-run reaper discards such checkpoints when failing the run (`unconfirmed_checkpoint_discarded` on the failure event). Out-of-tree integrations that call `gate.Resume` + `engine.ContinueAfterCheckpoint` directly with a custom gate must call `engine.ClearPendingPauseMarker` after a successful resume.
- **JSON built with `fmt.Sprintf("%q")` / `strconv.Quote`**: every JSON-producing site (checkpoint/resume snapshot variables, workflow tool-approval checkpoint variables, `{"error":...}` event payload fallbacks) now uses `json.Marshal`; `%q` emits Go-only `\xNN` escapes that are invalid JSON for strings containing control characters.
- **Approved-resume tool results**: the HITL approved-tool path now materializes its LLM-facing message through the same `materializeToolBatchItem` as the normal batch path, so truncation metadata (`tool_result_class` / `truncate_strategy`) matches; `ToolOutputMaxBytes` truncation already applied there.
...
- Weighted `observability.Recorder.AddCounter` updates for token and byte totals, plus production webhook signature verification wiring.
