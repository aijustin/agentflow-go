...
- **MCP multi round-trip requests (MRTR) and elicitation.** Protocol revision `2026-07-28` replaces server-initiated callbacks with a stateless flow: rather than completing, a server may answer `tools/call` with an `InputRequiredResult` carrying the requests it needs answered plus an opaque `requestState`, and the client re-issues the original call with `inputResponses` and the echoed state so any instance can resume it. Both transports now drive that loop. Hosts answer by supplying an `mcp.Elicitor` (`httpx.MCPRegistry.Elicitor`); the `elicitation` capability is derived from its presence, because the specification only lets a server ask for input the client declared. Requests arrive with their mode intact (`form`, or `url`, which servers must use for credentials). Fails closed: an unconfigured elicitation, or a request for the `sampling/createMessage` and `roots/list` methods deprecated in the same revision, is an error rather than a guess. Round trips are bounded by `MaxInputRounds` (default 8), and MRTR is only driven in `ProtocolModeModern`. See [mcp-tools.md](docs/mcp-tools.md#multi-round-trip-requests-与-elicitation).
