...
| 🟠 中 | D3 | 用 `fmt.Sprintf("%q")` 构造 JSON，可能产出非法 JSON |
| 🟠 中 | D4 | HITL 批准恢复路径未应用 `ToolOutputMaxBytes` 截断 |
| 🟠 中 | D5 | 审批暂停-恢复后，该轮 assistant + tool results 可能漏写 conversation memory |
| 🟠 中 | D6 | detached stream 取消观察者每 100ms 全量 `LoadAuthorized`，DB 压力大 |
| 🟠 中 | D7 | checkpoint 写入与 `gate.Pause` 之间崩溃会留下 Running + 悬挂 checkpoint，且恢复不校验 gate token |
...
- **位置**：final output 组装、`saveCheckpointVariables`、部分 event payload、`variableString` 兜底等多处。
...
- **位置**：`runtime_continue.go` 恢复路径对 `pending[1:]` 传 `persistTurnMemory=false`，只写 StepOutputs。
