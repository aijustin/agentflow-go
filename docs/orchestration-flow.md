...
本文档描述 agentflow-go 从 Scenario 定义到 Run 完成的完整执行链路，涵盖 **Skills 展开、LLM 调用、Tools 执行、三种编排模式、HITL 暂停/恢复**，以及 **[编排模式与节点选型指南](#九编排模式与节点选型指南)**（各 mode / 节点适用场景）。

相关代码入口见文末「关键代码索引」。字段级说明见 [configuration-reference.md](./configuration-reference.md)。

---

## 一、总览：从 Scenario 到 Run 完成

```mermaid
flowchart TB
    subgraph S1["① 配置加载与展开"]
        SCN["pkg/builder / core.Scenario"]
        LOAD["ValidateScenario"]
        VAL1["Validate<br/>引用/Workflow DAG 校验"]
        ASB["appscenario.Build"]
...
sequenceDiagram
    ...
    U->>F: builder stack or ValidateScenario
...
|----------|-----------|-----------------|------|
| 开放式问答、动态 tool 选择 | `autonomous` | `planning`、memory | `builder.MinimalAutonomous("assistant")` |
| 预先写死的 DAG | `fixed_workflow` | `tool` → `transform` → `agent` | `builder.MinimalFixedWorkflowReview("reviewer")` |
| 可复用子流程（对齐 LangGraph subgraph） | `fixed_workflow` | **`subgraph`** + `orchestration.workflows` | [orchestration-parity.md](./orchestration-parity.md) |
| 先结构化步骤，再 LLM 综合 | `hybrid` | Phase-1 workflow，Phase-2 autonomous | `builder.HybridResearch("analyst")`、`builder.MultiExpertResearch()` |
| Agentic RAG | `fixed_workflow` | `query_router`、`rag_grade` | `builder.AdaptiveRAG("assistant")` |
...
**示例**：`builder.MinimalAutonomous("assistant")`、`builder.MinimalHumanInLoop("assistant")`、`builder.ContextGovernance("assistant")`
...
**示例**：`builder.CodeReviewPipeline()`、`builder.WorkflowEnhancements()`、`builder.CorrectiveRAG("assistant")`
...
**可叠加**：`planning.enabled` 可在 Phase-2 前基于 workflow 上下文再生成计划（见 `builder.MultiExpertResearch()`）。

**示例**：`builder.HybridResearch("analyst")`、`builder.MultiExpertResearch()`
...
- **示例**：`builder.MultiExpertResearch()`
...
- **示例**：`builder.AdaptiveRAG("assistant")`
...
- **示例**：`builder.CorrectiveRAG("assistant")`、`builder.SelfRAG("assistant")`
...
- **示例**：`builder.MinimalTicketHandling("support")`
...
## 十、实例：`builder.CodeReviewPipeline()`

`builder.CodeReviewPipeline()` 是 **fixed_workflow** 模式，对应第四节节点调度：
