...
该目标会执行 `fmt`、`test`、`vet`、`security`（govulncheck）、`validate-builder`（`CoreCatalog` autonomous 核心栈）、`validate-catalog`（`examples/catalog/` 下 tool/skill manifest）。全量 `ExampleCatalog`（含 legacy workflow/hybrid）可用 `go run ./examples/go/validate -kind builder full` 手工校验。
