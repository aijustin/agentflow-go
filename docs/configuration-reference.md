...
LLM Profile 定义在 `scenario.llms.<name>` 下，可被 Agent 或 Tool 引用。
...
| `context_window_tokens` | 整数 | 否 | 该 Profile 声明的上下文窗口大小。 |
...
| `reasoning_effort` | 字符串 | 否 | Provider 特定的 reasoning 强度提示，例如 `low`、`medium`、`high`。 |
| `context` | 对象 | 否 | 上下文窗口治理策略。 |
| `extra_body` | 对象 | 否 | Provider 特定的请求体扩展字段。 |
| `capabilities` | 字符串数组 | 否 | 显式声明该 Profile 支持的能力。 |
...
Memory 定义在 `scenario.memories.<name>` 下，可被 Agent 引用。

| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `type` | 枚举 | 是 | Memory 后端类型。 |
| `scope` | 枚举 | 是 | Memory 命名空间作用域。 |
...
| 值 | 含义 |
| --- | --- |
| `in_memory` | 进程内临时 Memory。 |
| `file` | 由宿主应用注入的文件持久化 Memory 仓库。 |
| `custom` | 宿主应用自行提供 Memory 仓库。 |
...
Tool 定义在 `scenario.tools.<name>` 下，只有被 Agent 显式列出的 Tool 才能被该 Agent 使用。

| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `type` | 字符串 | 是 | Tool executor 类型。内置示例包括 `builtin.echo`、`builtin.http`、`builtin.filesystem`、`builtin.sql`、`mcp.tool`；应用也可以注册自定义工具。 |
| `description` | 字符串 | 否 | Tool 的用途说明。 |
| `input_schema` | 对象 | 否 | Tool 输入的 JSON Schema 片段。 |
| `output_schema` | 对象 | 否 | Tool 输出的 JSON Schema 片段。 |
| `side_effect` | 枚举 | 否 | 用于治理判断的副作用等级。 |
| `approval` | 枚举 | 否 | Tool 执行前的审批策略。 |
| `llm` | 字符串 | 否 | 可选 LLM Profile 覆盖。 |
| `rate_cap` | 整数 | 否 | 单次 run 内该 Tool 的最大调用次数。 |
| `metadata` | 字符串映射 | 否 | 运维侧自定义元数据。 |
...
| `llm` | 字符串 | 否 | 来自 `scenario.llms` 的 Profile 名称。 |
| `memory` | 字符串 | 否 | 来自 `scenario.memories` 的 Memory 名称。 |
| `tools` | 字符串数组 | 否 | 来自 `scenario.tools` 的 Tool 名称。 |
...
| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `mode` | 枚举 | 否 | Runtime 编排模式。为空时按 autonomous 行为处理。 |
...
Runtime 设置定义在 `scenario.runtime` 下。

| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `timeout` | duration | 否 | 全局 run 超时时间。 |
