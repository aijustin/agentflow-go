...
- Structured output runs use an agent-level `output_schema` and provider `StructuredOutputter`; plain streaming uses `Streamer`; tool-enabled `Stream` runs the governed tool loop to completion and delivers one final `Done` chunk (persisted). `before_final_answer` and `fixed_workflow` graphs with `agent` nodes are not supported on `Stream`/`RunStructured`.
