...
When an agent has tools and the configured LLM gateway supports `CapToolCall`, the runtime runs an autonomous tool loop: send tool specs to the LLM, validate returned tool calls against the agent whitelist, enforce approval and per-run `rate_cap`, retry classified transient LLM/tool errors from `retry_limit`/`max_retries` with exponential backoff (`write`/`external`/`dangerous` tools are not auto-retried by default), execute registered tool executors, append bounded tool results, and continue until the LLM returns a final answer or `max_steps` is reached.

`Stream` also accepts tool-enabled agents: it runs the same governed tool loop as `Run`, emitting ordered `kind=tool_call` / `tool_result` (or `tool_denied`) progress chunks during the loop, then delivers the final answer as a terminal `Done` chunk (progress is not part of the persisted final output). Each model turn still uses blocking `ChatWithTools` (not provider-level streamed tool_call tokens). Agents with a `before_final_answer` HITL checkpoint are rejected by `Stream` (use `Run` / `RunStructured`); tool `approval: pause` can still surface via a terminal `Paused` chunk. `RunStructured` / `Stream` on a `fixed_workflow` that contains `agent` nodes are rejected so agents are not executed twice.
...
Mount library handlers in your own HTTP server. The example listens on `127.0.0.1:7060`:
...
- Structured output runs use an agent-level `output_schema` and provider `StructuredOutputter`; plain streaming uses `Streamer`; tool-enabled `Stream` emits incremental `tool_call`/`tool_result` (or `tool_denied`) chunks during the governed tool loop and delivers a terminal `Done` chunk for the final answer (persisted). `before_final_answer` and `fixed_workflow` graphs with `agent` nodes are not supported on `Stream`/`RunStructured`.
...
**Current release: v0.5.1** — on top of v0.5.0, keeps successful business tool observations visible when governance denials would otherwise advance the observation mask. See [CHANGELOG.md](CHANGELOG.md) for details.
